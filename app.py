...
with st.sidebar:
    ...
    edge_mode     = st.selectbox("Detection Method",
...
if uploaded_file is None:
    ...
    if edge_mode == "Canny Edges":
        ...
        processed, sobel_x, sobel_y, sobel_combined = sobel_edges(clean_for_edges)
        findings.append("Sobel edge detection completed — directional gradients computed")
        findings.append("  Red channel = vertical edges (X gradient) | Blue = horizontal (Y gradient)")
        techniques_applied.append({
            "name": "Sobel Edge Detection",
            "params": "ksize=3, X and Y gradients, colour-coded output"
        })
        ...
        if impact_count > 0:
            ...
            findings.append("  Impact detection uses circularity ratio — circle=1.0, threshold>0.65")
        if areas:
        ...
        techniques_applied.append({
            "name": "Contour Detection + Impact Analysis",
            "params": f"min_area={min_area}px, circularity threshold=0.65"
        })
    ...
    if enable_color:
        ...
        techniques_applied.append({
            "name": "HSV Colour Segmentation (Multi-Range)",
            "params": f"4 blood ranges + user H({h_min}-{h_max}) S({s_min}-{s_max}) V({v_min}-{v_max})"
        })
    ...
    if apply_face:
        ...
        techniques_applied.append({
            "name": "Face Detection (YuNet + Haar)",
            "params": "YuNet confidence>45%, Haar scaleFactor=[1.1,1.2], minNeighbors=8"
        })

    # ── Composite: redraw stain borders on top if both ran ────────────────────
    if enable_color and apply_face and mask_cv is not None and face_details_list:
        stain_contours, _ = cv2.findContours(
            mask_cv, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        cv2.drawContours(processed, stain_contours, -1, (0, 0, 255), 2)
